#!/usr/bin/env python
"""Ubica archivos de ``99_Nuevas_Secciones`` en su destino definitivo."""
import shutil
    ...
    for md in nuevas_dir.glob("*.md"):
        ...
        for sec in index_data["secciones"]:
            if normalizar(sec["titulo"]) in nrm_nombre:
                nuevo_path = (
                    destino_base / f"{sec['id']}_{sec['titulo'].replace(' ', '_')}.md"
                )
            ...
            if "subtemas" in sec:
                for sub in sec["subtemas"]:
                    if normalizar(sub) in nrm_nombre:
                        folder = (
                            "02_Instancias_SQL"
                            if "sql" in normalizar(sub)
                            else sec["slug"]
                        )
                        nuevo_path = (
                            destino_base / folder / f"{sub.replace(' ', '_')}.md"
                        )
        ...
        if not encontrado:
            ...
            for sec in index_data["secciones"]:
                candidates.append(
                    (
                        SequenceMatcher(
                            None, nrm_nombre, normalizar(sec["titulo"])
                        ).ratio(),
                        sec["titulo"],
                    )
                )
                for sub in sec.get("subtemas", []):
                    candidates.append(
                        (
                            SequenceMatcher(None, nrm_nombre, normalizar(sub)).ratio(),
                            sub,
                        )
                    )
